...
knitr::kable(StormData[sample(nrow(StormData), 8),])
```
...
knitr::kable(StormData_SUMMARY[1:5, 1:5])
knitr::kable(StormData_SUMMARY[1:5, c(1:3, 6:7)])
```
...
We can now arrange the `StormData_SUMMARY` so that becomes clear which event types cause the most fatalities, injuries, property and crop damage. But first let's order the list by number / percentage of occurences.
...
Using the ordered lists we can deduce that **`r StormData_COUNT[1,"EVTYPE"]`** is the event type that occurs most often. The top 5 looks like this: `r tolower(StormData_COUNT[1:5,"EVTYPE"])`.
...
```{r stacked_barplot_fatalities&injuries, fig.cap = "CAPTION: Barplot showing total fatalities plus injuries for each event type.", fig.fullwidth = TRUE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r dotplot_fatalities&injuries, fig.cap = "CAPTION: Dotplot showing total injuries versus total fatalities; each dot represents an event type.", fig.fullwidth = TRUE, message=FALSE, warning=FALSE, cache=TRUE}
...
```{r stacked_barplot_property&crop_damage, fig.cap = "CAPTION: Barplot showing crop damages plus property damages for each event type.", fig.fullwidth = TRUE, message=FALSE, warning=FALSE, cache=TRUE}
# Source of this method:
# https://stackoverflow.com/questions/30949896/creating-stacked-barplots-in-r-using-different-variables
...
Tornadoes are most harmful to population health. Floods have the greatest economic consequences, although tornadoes cause significant property damage too.

## Comments on HTML-output

- The title can not be split over two lines, which would have been desireable in this case (long titles are less readable).
- The default setting makes for long lines of text / a wide view, which makes the whole document less readable.
- That the R-code is displayed in wide format is great, because one can easily read all of it without scrolling.
- When R results are displayed and information does not fit within the (width of the) results window it becomes longer, which messes up the flow of the text. This is annoying. Especially when the remaining information fills only a small portion of the (width of the) results window.
- The available headings do not differ enough, leaving us with a very chunky layout.
- The figure captions cannot be fine-tuned enough. The default placement, size and markup are awkward.

Possible solutions?

- Making the width of the text smaller (Tufte style).
- Hiding most of the code chunks if they are wider / longer, while providing the user with an option to expand the window. This way wider / longer chunks won't disrupt the flow of the text so much (short and narrow chunks won't need expanding, of course). When there's no need for a wide code-window, it won't be displayed.
- Figure captions should be displayed in the sidebar (Tufte style), or clearly recognizable as captions when displayed above / below figures. 

## Comments on HTML-output (Tufte)

- The narrower display of text makes for easier reading.
- The size of the type inside of the code chunks is (way) to big.
- The way code gets forced into a narrow window seems unnatural. The abundance of scroll bars disrupts the flow of the text.
- Code that does not need to be squeezed (no scroll bar) blends into the regular text to much. The same goes for inline code and results (R output).
- The headings are too course; subheadings are too small.
- The light yellow background does not match with the background of plots.
- The serif font seems out of place for text that will be displayed on a screen.

Possible solututions?

- Adjusting the Tufte style. Actually it seems like merging elements of the regular style with elements from tufte is what I want.

See this [link](https://rstudio.github.io/tufte/) for more info about tufte styled output. And [here](https://github.com/edwardtufte/tufte-css) for the original tufte css.

## Software Environment

```{r print_software_environment, message=FALSE, warning=FALSE, cache=TRUE}
sessionInfo()
```